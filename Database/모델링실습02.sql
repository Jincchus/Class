...
INSERT INTO `Departments` VALUES (112,'치과','정봉준','051-123-0112');
...
INSERT INTO `Doctors` VALUES ('D103101','장보고','1979-07-28','M',103,'과장','010-3101-1979','jagbg@bw.com');
...
INSERT INTO `Doctors` VALUES ('D107103','이이','1992-0907','M',107,'전문의','010-7103-1992','leelee@bw.com');
...
INSERT INTO `Nurses` VALUES ('N108101','송중기','1992-10-07','M',108,'간호사','010-710-9210','jungi@bw.com');
...
SELECT 
	...
	`dep_name`
from `nurses` 
 JOIN `departments` AS b USING(`dep_no`);
...
#문제4. 모든 진료내역에서 환자 이름, 담당의사 이름, 진료내용, 처방내용, 진료날짜를 조회사시오.
SELECT
	`pat_name`,
	b.`doc_name`,
	a.`treat_contents`,
...
JOIN `doctors` AS b ON a.doc_id = b.doc_id
JOIN `charts` AS c ON a.treat_no = c.treat_no
...
SELECT GROUP_CONCAT(`pat_name` SEPARATOR '^') AS `pat_name`
FROM `nurses` as a
JOIN `patients` AS b ON a.nur_id = b.nur_id
WHERE nur_name = '김태희';

#문제10. 가장 많은 환자 수를 담당하는 간호사 ID, 간호사 이름, 담당환자수를 조회 하시오.
SELECT 
	a.nur_id,
	nur_name,
	COUNT(*) AS `환자수`
