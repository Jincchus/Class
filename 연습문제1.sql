...
CREATE TABLE `Product`(
	`prodNo`		INT PRIMARY KEY AUTO_INCREMENT NOT NULL,
...
SELECT `orderProduct` AS `주문 상품번호`, SUM(`orderCount`) AS `총 주문수량`
 FROM `Order` GROUP BY `orderProduct` ORDER BY`orderProduct` ASC ;
...
#실습1-29
SELECT a.orderId, `prodName` FROM `order` AS a 
JOIN `product` AS b ON a.orderProduct = b.prodNo WHERE `orderid` = 'c102';
